...
const assetsPath = 'C:\\Users\\Abhishek\\portfolio\\assets-b64.json';
const rawData = fs.readFileSync(assetsPath, 'utf8').replace(/^\uFEFF/, '');
const assets = JSON.parse(rawData);

const htmlContent = `<title>Abhishek Kumar Pramanik</title>
...
<main>
  ...
  <section id="about">
    ...
    <div class="about-grid">
      <div>
        ...
        <p class="about-text">
          B.Tech Computer Science & Engineering student at Lovely Professional University focused on building full-stack applications, AI-powered systems, and technology-driven projects. Passionate about solving real-world challenges with robust software and modern AI workflows.
        </p>
      ...
      <div class="photo-frame" data-cursor="ABHISHEK">
        <img src="${assets.photo}" alt="Abhishek Kumar Pramanik">
  ...
  <section id="certifications">
    ...
    <div class="cert-grid">
      ...
      <div class="cert-card" data-cursor="INFOSYS">
      ...
      <div class="cert-card" data-cursor="SAILOR ACADEMY">
...
<div class="modal-backdrop" id="modal" onclick="closeModal()">
  <button class="modal-close" onclick="closeModal()" data-cursor="CLOSE">Close [ESC]</button>
  <div class="modal-inner" id="modal-content" onclick="event.stopPropagation()"></div>
</div>
...
function openProject(key) {
  ...
  modalContent.innerHTML = \`
    <div style="font-family: var(--font-mono); font-size: 0.75rem; color: var(--accent); margin-bottom: 1.5rem;">\${p.tag}</div>
    <h2 style="font-size: clamp(2rem, 5vw, 4rem); font-weight: 400; margin-bottom: 2rem;">\${p.title}</h2>
    ...
    <div style="display: flex; gap: 1rem; border-top: 1px solid var(--border); padding-top: 2rem;">
      \${p.github ? \`<a href="\${p.github}" target="_blank" rel="noopener noreferrer" style="font-family: var(--font-mono); font-size: 0.75rem; text-transform: uppercase; padding: 0.8rem 1.6rem; border: 1px solid var(--border); color: #fff; text-decoration: none;">View Repository ↗</a>\` : \`<span style="font-family: var(--font-mono); font-size: 0.75rem; text-transform: uppercase; padding: 0.8rem 1.6rem; border: 1px solid var(--border); color: var(--text-muted);">Repository Coming Soon</span>\`}
      \${p.live ? \`<a href="\${p.live}" target="_blank" rel="noopener noreferrer" style="font-family: var(--font-mono); font-size: 0.75rem; text-transform: uppercase; padding: 0.8rem 1.6rem; border: 1px solid var(--accent); color: var(--accent); text-decoration: none;">Live Demo ↗</a>\` : \`<span style="font-family: var(--font-mono); font-size: 0.75rem; text-transform: uppercase; padding: 0.8rem 1.6rem; border: 1px solid var(--border); color: var(--text-muted);">Live Demo Coming Soon</span>\`}
    </div>
  \`;
  ...
  if (type === 'techveda') {
    modalContent.innerHTML = \`
      <div style="text-align: center;">
        <img src="\${assets.cert}" alt="Tech Veda Certificate" style="max-width: 100%; max-height: 80vh; border: 1px solid var(--border); object-fit: contain;">
      </div>
    \`;
    modal.classList.add('open');
  }
...
fs.writeFileSync('C:\\Users\\Abhishek\\portfolio\\shareable.html', htmlContent, 'utf8');
